...
<section class="login" id="login">
    <div class="heading">
       <h3>Informe Producto</h3>
    </div>

    <?php


echo '<br><br><center><h1>Puede ver la información del producto, si desea puede descargar el informe en ecxel
    ...
    echo '<center><tr><th>ID</th><th>IMAGEN</th><th>NOMBRE</th><th>CANTIDAD</th><th>DESCRIPCION</th><th>PRECIO</th><th>IDTIPO</th><th>IDPROVEEDOR</th><th>IDFACTURA</th><th>Eliminar</th></tr></center>';
    while ($filas= mysqli_fetch_row($resultado))
     {
        echo '<tr><td>'.$filas[0].'</td><td>'.$filas[1].'</td><td>'.$filas[2].'</td><td>'.$filas[3].'</td> <td>'.$filas[4].'</td><td>'.$filas[5].'</td><td>'.$filas[6].'</td><td>'.$filas[7].'</td><td>'.$filas[8].'</td>
        <td><form action="eliminar2.php" method="post">
     ...
     echo ('<center><br><br><form action="generarxls2.php" method="post" class="login-form"><input type="submit" value="GENERAR_XLS" class="btn"> 
     </form></td></tr></center>');
